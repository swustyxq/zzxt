...
$(document).ready(function(){
	...
	$.ajax({
		...
		success : function(msg) {
			if(msg.result == true){
				...
				if(msg.user.userType == 1||msg.user.userType == 2){
					changebutton();
				}else{
	...
	$("#pass").click(function(){
		if(check()){
			//alert(check());
			insertparent(studNumber);//添加父母信息
			var state = true;
			insert(state,studId,studNumber); 
	...
	$("#reject").click(function(){
		$.ajax({
			type:"post",
			contentType : "application/x-www-form-urlencoded;charset=UTF-8",
			url : '/zzxt/lstdController/rejectToStudent.do',
			async : false,
			data : {
				studId:studId
			},
...
function insert(state,studId,studNumber){
	...
	var academyopinion ="";
	if(user.userType == 6){
		academyopinion = $("#academyopinion").val();
	}else{
		academyopinion = "";
	}
	var lstdLoans="";//贷款方式
...
if(msg.lstd){
		...
		if(user.userType != 6){
			$("#academyopinion").attr("disabled","disabled");
		}
...
function insertparent(studNumber){
	//var studNumber = $("#stuNumber").val();
